...
export default function EditPowerMeter({ setType, doubleCheck }: props) {
    ...
    return (
        <>
            ...
            <div className={`${pmInput.allocation ? 'mb-10' : 'mb-30'}`}>
            ...
            <div className="mb-40 flex justify-between">
                <p>Status Active</p>
                <button
                    className={`relative w-100 h-35 rounded-full transition-colors duration-300 ${pmInput.isActive ? "bg-cGreen2" : "bg-cRed"}`} onClick={() => setPmInput(prev => ({...prev, isActive: !prev.isActive}))}>
                    <div className={`absolute top-5 w-25 h-25 bg-white rounded-full transition-transform duration-300 ${pmInput.isActive ? "translate-x-70" : "translate-x-5"}`} />

                    <span className={`absolute top-1/2 -translate-y-1/2 transition-all duration-300 ${pmInput.isActive ? "left-16 text-black" : "right-10 text-white"}`}>
                        {pmInput.isActive ? "Active" : "Inactive"}
                    </span>
                </button>
            </div>
